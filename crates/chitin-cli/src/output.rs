...
mod tests {
  ...
  fn existing_directory_should_receive_generated_filename() {
    ...
    let resolved = resolve_output_path(Some(directory.clone()), &test_id(), StructureFormat::Pdb);
  ...
  fn nonexistent_extensionless_output_should_be_used_as_file() {
    ...
    let resolved = resolve_output_path(Some(path.clone()), &test_id(), StructureFormat::Pdb);
  ...
  fn path_with_extension_should_be_used_as_file() {
    ...
    let resolved = resolve_output_path(Some(path.clone()), &test_id(), StructureFormat::Mmcif);
    assert!(matches!(resolved, Ok(resolved) if resolved == path));
  }
}
