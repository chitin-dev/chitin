...
use super::{ChainId, ElementCategory, ResidueKind, StructureScene};
...
fn contour_field(bounds_min: glam::Vec3, spacing: f32, dimensions: [usize; 3], field: &[f32]) -> SurfaceMesh {
...
fn probe_surface_distance(position: glam::Vec3, probe_grid: &PointGrid, probe_radius: f32, spacing: f32) -> f32 {
  let limit = DISTANCE_FIELD_RANGE * spacing;
  let mut distance = limit;
...
impl AtomGrid {
  ...
  fn new(atoms: Vec<SurfaceAtom>) -> Self {
  ...
  fn for_each_nearby(&self, position: glam::Vec3, radius: f32, mut visit: impl FnMut(&SurfaceAtom)) {
...
fn vertex_position(vertex: &[f32; 6]) -> glam::Vec3 {
...
fn retain_inner_components(mesh: SurfaceMesh, atom_grid: &AtomGrid, probe_radius: f32) -> SurfaceMesh {
...
fn smooth_mesh(mesh: &mut SurfaceMesh, iterations: usize) {
  if mesh.indices.is_empty() {
    return;
  }
